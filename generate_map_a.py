"""
Weather Map Generation script for location a: Limboto - Bolango - Bone Basin Area
"""
...
logging.basicConfig(filename="rainfall_maps_a.log", level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
...
LOCATIONS = [
    (0.507739,123.224852),(0.531979,122.720655),(0.502984,123.626910),(0.777981,122.856571),
    (0.539958,122.977448),(0.378843,123.420817),(0.583394,123.422561),(0.700981,123.164251),
    (0.671768,122.858140),(0.648886,123.295000),(0.570415,123.107009),(0.392709,123.284547),
    (0.524208,122.850680),(0.662549,123.011900),(0.472134,123.499119),(0.481546,123.363297),
    (0.752228,123.080434),(0.602480,123.200672),(0.565496,123.328449),(0.560916,123.509104),
    (0.914343,123.047961),(0.816298,122.960883),(0.476242,123.138941)

]

MAP_CENTER = [0.648886,123.295000]
ZOOM_START = 10
# ===========================================
...
def create_prediction_map():
    ...
    if values_peak:
        ...
        add_idw_overlay(fg_idw_peak, points_peak, values_peak, "idw_peak_a", "YlOrRd", "IDW Peak Rain (mm)_a", 
                        vmin=vmin_p, vmax=vmax_p)
        create_colorbar_png("colorbar_peak_a.png", "YlOrRd", vmin_p, vmax_p, "Peak Rainfall (mm)_a")
    ...
    if values_24h:
        ...
        add_idw_overlay(fg_idw_24h, points_24h, values_24h, "idw_24h_a", "YlGnBu", "IDW Expected 24h (mm)_a", 
                        vmin=vmin_24, vmax=vmax_24)
        create_colorbar_png("colorbar_24h_a.png", "YlGnBu", vmin_24, vmax_24, "Expected 24h Rainfall (mm)_a")
...
def create_realtime_map():
    ...
    add_idw_overlay(fg_idw, points, values, "idw_current_a", "YlGnBu", "Current 1h Rainfall_a")

    rmax = max(values) if values else 0
    vmax = max(5.0, round(rmax + 0.8, 1)) if rmax > 0 else 3.0
    create_colorbar_png("colorbar_current_a.png", "YlGnBu", 0.0, vmax, "Current 1h Rainfall (mm)_a")
    ...
    current_b64 = embed_image_to_base64("colorbar_current_a.png")
    legend_html_real = legend_html_real.replace("[CURRENT_BASE64]", current_b64)

    add_draggable_legend(m, legend_html_real, "legend-real")

    folium.LayerControl().add_to(m)
    m.save("rainfall_realtime_map_a.html")
    print("✅ Saved: rainfall_realtime_map_a.html")
...
if __name__ == "__main__":
    ...
    print("  • rainfall_prediction_map_a.html")
